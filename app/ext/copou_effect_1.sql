...
join hawkeye.zhouweinan_256 b on a.user_id=b.member_no --s-- 关联活动表 仅查询当前活动的效果
where a.activity_coupon_id in (407,408)    
  and to_date(a.create_time)>='2016-10-04' --s-- 券发放开始时间        
  and to_date(a.create_time)<='2016-10-04' --s-- 券发放结束时间
group by a.activity_coupon_id
        ,b.member_no
;

--临时表:发券客户交易表
drop table bi.wy_effect_trans;
create table bi.wy_effect_trans as
select b.member_no
      ,d.activity_coupon_id as trans_coupon_id
      ,a.activity_coupon_id as issue_coupon_id
      ,case when d.activity_coupon_id in (407,408) then 1 else 0 end is_coupon_trans --s-- 券ID
...
where b.status in (6,8,9,11)
  ...
  and to_date(b.trans_time)>='2016-10-04' --s-- 券使用查询开始时间
  and to_date(b.trans_time)<='2016-10-07' --s-- 券使用查询结束时间
;
...
from bi.wy_effect_trans where fund_trans_type=1
...
select 'TOTAL' as activity_coupon_id
      ,sum(fund_trans_amount) as trans_amt
      ,count(distinct member_no) as trans_user_cnt
from(select member_no, trans_time, fund_trans_amount from bi.wy_effect_trans where fund_trans_type=1 group by member_no, trans_time, fund_trans_amount)t
)t2 on t1.activity_coupon_id=t2.activity_coupon_id
left outer join
(--用券投资额 用券投资人数 券使用张数 折扣金额 加息金额
select 'TOTAL' as activity_coupon_id
...
from bi.wy_effect_trans
where is_coupon_trans=1 and trans_coupon_id=issue_coupon_id and fund_trans_type=1
)t3 on t2.activity_coupon_id=t2.activity_coupon_id
left outer join
(--用券人全部投资额 
select 'TOTAL' as activity_coupon_id
      ,count(*) as coupon_trans_cnt_all
...
where to_date(b.trans_time)>='2016-10-04'
  and to_date(b.trans_time)<='2016-10-07'
